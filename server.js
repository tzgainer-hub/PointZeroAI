...
// ── TRIPSIGNAL ROUTES ──
app.get('/tripsignal', (req, res) => res.sendFile(path.join(__dirname, 'public', 'tripsignal', 'index.html')));
app.get('/tripsignal/privacy', (req, res) => res.sendFile(path.join(__dirname, 'public', 'tripsignal', 'privacy.html')));
...
// ── TRIPSIGNAL WAITLIST SUBMIT ──
app.post('/api/tripsignal-waitlist', async (req, res) => {
  try {
    const rawEmail = (req.body && req.body.email) ? String(req.body.email).trim() : '';
    if (!rawEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(rawEmail)) {
      return res.status(400).json({ ok: false, error: 'Invalid email' });
    }
    console.log('TripSignal waitlist signup:', rawEmail);

    // Notify Tom
    await sendResendEmail({
      to: 'tomz@pointzeroai.com',
      subject: `\u{1F4F1} TripSignal waitlist signup: ${rawEmail}`,
      html: `
        <div style="font-family:Inter,Arial,sans-serif;max-width:600px;margin:0 auto;background:#0c1e3c;color:#fff;padding:32px;border-radius:8px;">
          <h2 style="color:#2dd4bf;margin:0 0 8px 0;font-family:'Space Grotesk',sans-serif;">New TripSignal waitlist signup</h2>
          <p style="color:rgba(255,255,255,0.7);margin:0 0 24px 0;font-size:0.9rem;">Someone just asked to be notified when TripSignal launches.</p>
          <table style="width:100%;border-collapse:collapse;">
            <tr><td style="padding:10px 0;color:#8a9bb5;width:120px;border-bottom:1px solid #1a2e4a;font-size:0.9rem;">Email</td><td style="padding:10px 0;font-weight:600;border-bottom:1px solid #1a2e4a;">${esc(rawEmail)}</td></tr>
            <tr><td style="padding:10px 0;color:#8a9bb5;font-size:0.9rem;">Source</td><td style="padding:10px 0;">pointzeroai.com/tripsignal</td></tr>
            <tr><td style="padding:10px 0;color:#8a9bb5;font-size:0.9rem;">Timestamp</td><td style="padding:10px 0;">${new Date().toISOString()}</td></tr>
          </table>
        </div>
      `
    });

    // Confirmation to the signup
    await sendResendEmail({
      to: rawEmail,
      subject: "You're on the TripSignal launch list.",
      html: `
<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>TripSignal launch list</title></head>
<body style="margin:0;padding:0;background:#f6f8fb;font-family:Arial,Helvetica,sans-serif;">
  <table role="presentation" align="center" cellpadding="0" cellspacing="0" border="0" width="100%" style="background:#f6f8fb;">
    <tr><td align="center" style="padding:32px 16px;">
      <table role="presentation" align="center" cellpadding="0" cellspacing="0" border="0" width="600" style="max-width:600px;background:#0c1e3c;border-radius:8px 8px 0 0;">
        <tr><td style="padding:28px 32px;">
          <div style="font-family:Arial,sans-serif;font-size:11px;letter-spacing:3px;text-transform:uppercase;color:#2dd4bf;font-weight:700;margin-bottom:6px;">TripSignal &middot; Point Zero AI</div>
          <div style="font-family:Arial,sans-serif;font-size:13px;color:rgba(255,255,255,0.75);">Launch list confirmation</div>
        </td></tr>
      </table>
      <table role="presentation" align="center" cellpadding="0" cellspacing="0" border="0" width="600" style="max-width:600px;background:#ffffff;border:1px solid #e5e9f0;border-top:none;border-radius:0 0 8px 8px;">
        <tr><td style="padding:36px 32px 8px;">
          <h1 style="font-family:Arial,sans-serif;font-size:24px;font-weight:700;color:#0c1e3c;margin:0 0 14px;line-height:1.3;">You're on the list.</h1>
          <p style="font-family:Arial,sans-serif;font-size:15px;color:#2d3748;line-height:1.7;margin:0 0 16px;">Thanks for signing up. We'll send you one email the day TripSignal goes live on the App Store &mdash; and that's it. No marketing churn.</p>
          <p style="font-family:Arial,sans-serif;font-size:15px;color:#2d3748;line-height:1.7;margin:0 0 16px;">In the meantime, you can read what TripSignal will do at <a href="https://www.pointzeroai.com/tripsignal" style="color:#0c1e3c;font-weight:600;border-bottom:1px solid #2dd4bf;text-decoration:none;">pointzeroai.com/tripsignal</a>.</p>
        </td></tr>
        <tr><td style="padding:24px 32px 32px;border-top:1px solid #e5e9f0;background:#f9fafb;border-radius:0 0 8px 8px;">
          <p style="font-family:Arial,sans-serif;font-size:13px;color:#6b7280;line-height:1.65;margin:0 0 14px;">Want off the list? Reply with "remove" and we'll take you off.</p>
          <p style="font-family:Arial,sans-serif;font-size:13px;color:#0c1e3c;font-weight:700;margin:0 0 4px;">&mdash; Point Zero AI LLC</p>
          <p style="font-family:Arial,sans-serif;font-size:11px;color:#6b7280;line-height:1.6;margin:0;">TripSignal is informational only. It is not medical advice and does not decide whether you should travel.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
    ...
    console.error('Server error in tripsignal-waitlist:', err);
    res.json({ ok: true });
  }
});
